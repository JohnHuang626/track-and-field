...
export default function TrackAndFieldManager() {
  ...
  const showMessage = (msg, isError = false) => {
    ...
    setTimeout(
      () => setToastMsg({ show: false, msg: '', isError: false }),
      3000
    );
  };
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        ...
        showMessage(
          '連線失敗：請至 Firebase 啟用「匿名 (Anonymous)」登入',
          true
        );
  ...
  const uniqueCompetitions = [...new Set(records.map((r) => r.competition))]
    .filter(Boolean)
    .sort();
  const uniqueEvents = [...new Set(records.map((r) => r.event))]
    .filter(Boolean)
    .sort();
  const uniqueAthletes = [...new Set(records.map((r) => r.athlete))]
    .filter(Boolean)
    .sort();
  ...
  const bestRecordsByEvent = useMemo(() => {
    ...
    records.forEach((record) => {
      if (!record.event || !record.score || record.score.toUpperCase() === 'X')
        return;
    ...
    for (const [event, items] of Object.entries(grouped)) {
      ...
      items.sort((a, b) =>
        isTrack ? a.scoreValue - b.scoreValue : b.scoreValue - a.scoreValue
      );
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <div
                                className={`w-8 h-8 rounded-full flex items-center justify-center text-xs border ${badgeBg}`}
                              >
                                {medalIcon}
                              </div>
                              <div>
      ...
      <div className="space-y-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <select
            ...
            className="w-full md:w-64 p-2.5 bg-gray-50 border border-gray-200 rounded-xl font-bold focus:ring-indigo-500"
          >
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
                ...
                <p className="text-2xl font-black text-indigo-600 mt-1">
                  {comps.size}{' '}
                  <span className="text-sm font-normal text-gray-500">場</span>
                </p>
              </div>
              <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
                ...
                <p className="text-2xl font-black text-amber-500 mt-1">
                  {golds}{' '}
                  <span className="text-sm font-normal text-gray-500">面</span>
                </p>
              </div>
              <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                <Medal className="text-amber-500" /> 個人最佳紀錄 (PB)
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {pbCards.map((pb) => (
                  <div
                    key={pb.event}
                    className="p-4 rounded-xl border border-indigo-100 bg-indigo-50/30 flex justify-between items-center"
                  >
                    <div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 overflow-x-auto">
              <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                <List className="text-gray-500" /> 參賽歷史明細
              </h3>
  ...
  const AnalyticsView = () => {
    ...
    const athleteEvents = [
      ...new Set(
        records.filter((r) => r.athlete === chartAthlete).map((r) => r.event)
      ),
    ];
    const [chartEvent, setChartEvent] = useState('');

    useEffect(() => {
      if (!chartAthlete && uniqueAthletes.length > 0)
        setChartAthlete(uniqueAthletes[0]);
    }, [uniqueAthletes]);
    useEffect(() => {
      ...
      if (active && payload && payload.length) {
        ...
        return (
          <div className="bg-slate-800 text-white p-3 rounded-lg shadow-xl border border-slate-600 text-sm">
            ...
            {data.rank && (
              <p className="text-amber-400 mt-1">名次: 第 {data.rank} 名</p>
            )}
          </div>
        );
      }
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-6">
        <div>
          <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2">
            <TrendingUp className="text-emerald-500" /> 選手成長趨勢分析
          </h2>
          <p className="text-sm text-gray-500">
            追蹤選手在不同賽事中的表現成長軌跡
          </p>
        ...
        {chartData.length > 0 ? (
          <div className="bg-slate-900/5 p-4 rounded-2xl border border-slate-200">
            <div className="flex justify-between items-center text-xs font-semibold text-gray-500 mb-4">
              ...
              <span>
                出賽紀錄: <strong>{chartData.length} 次</strong>
              </span>
            </div>
  ...
  const AllRecordsView = () => {
    ...
    const filteredRecords = records
      .filter((r) => {
        const matchSearch =
          r.athlete.includes(searchTerm) || r.competition.includes(searchTerm);
    ...
    const saveEdit = async () => {
      if (
        !editForm.competition ||
        !editForm.event ||
        !editForm.athlete ||
        !editForm.score
      ) {
        showMessage('請填寫必填欄位', true);
        return;
      }
      try {
        const docRef = doc(
        ...
        console.error(err);
        showMessage('更新失敗', true);
      }
    };

    const deleteRec = async (id) => {
      if (!window.confirm('確定要刪除這筆紀錄嗎？資料將從雲端永久刪除。'))
        return;
      try {
        const docRef = doc(
          db,
    ...
    return (
      <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
        <div className="flex flex-col lg:flex-row justify-between mb-6 gap-4">
          <div>
            <h2 className="text-2xl font-bold flex items-center gap-2">
              <List className="text-indigo-600" /> 雲端資料庫總覽
            </h2>
            <p className="text-sm text-gray-500 mt-1">
              共 {filteredRecords.length} 筆資料
            </p>
          </div>
          <div className="flex flex-col sm:flex-row gap-2.5">
            <div className="relative">
              ...
              <input
                ...
                className="pl-9 pr-3 py-2 bg-gray-50 border rounded-xl text-sm w-full"
              />
            </div>
            <select
              value={filterComp}
        ...
        <div className="overflow-x-auto rounded-xl border border-gray-100">
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            <thead className="bg-gray-50 text-gray-600 font-bold">
              <tr>
                <th className="px-6 py-3.5 text-left">賽事</th>
                <th className="px-6 py-3.5 text-left">項目</th>
                <th className="px-6 py-3.5 text-left">姓名</th>
                <th className="px-6 py-3.5 text-left">成績</th>
                <th className="px-6 py-3.5 text-left">名次</th>
                ...
                return (
                  <tr key={r.id} className="hover:bg-gray-50">
                    {isEditing ? (
                      ...
                      <>
                        <td className="px-6 py-4">{r.competition}</td>
                        <td className="px-6 py-4 font-bold text-indigo-600">
                          {r.event}
                        </td>
                        <td className="px-6 py-4 font-semibold">{r.athlete}</td>
                        ...
                        <td className="px-6 py-4 text-right">
                          <button
                            ...
                            className="text-indigo-600 p-1 mr-1"
                          >
                            <Edit2 size={16} />
                          </button>
                          <button
  ...
  const AddManageView = () => {
    ...
    const exportCSV = () => {
      ...
      link.href = url;
      link.download = `田徑備份_${new Date().toISOString().slice(0, 10)}.csv`;
      link.click();
    };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
          <h2 className="text-2xl font-bold mb-6 flex items-center gap-2">
            <PlusCircle className="text-indigo-600" /> 新增成績至雲端
          </h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, athlete: e.target.value })
                  }
                />
                <datalist id="dl-aths">
                  {uniqueAthletes.map((c) => (
                    <option key={c} value={c} />
                  ))}
                </datalist>
              </div>
            ...
            <button
              type="submit"
              disabled={!user}
              className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-400 text-white font-bold py-3.5 rounded-xl shadow-md flex justify-center items-center gap-2 transition"
            >
              <Save size={18} /> 儲存成績
            </button>
          </form>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 flex flex-col justify-between">
          <div>
            <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
              <Download className="text-emerald-500" /> 資料備份與轉移
            </h3>
            <p className="text-xs text-gray-500 mb-6 leading-relaxed">
              資料已安全儲存於 Firebase 雲端。您可以匯出備份，或批量匯入歷史資料
              (將附加至現有庫中)。
            </p>
            <div className="space-y-3">
              <button
                onClick={exportCSV}
                className="w-full bg-emerald-50 text-emerald-700 font-bold py-2.5 px-4 rounded-xl border border-emerald-200 text-sm flex justify-center items-center gap-2"
              >
          ...
          <div className="mt-8 p-4 bg-slate-50 rounded-xl text-xs text-slate-500 border border-slate-100">
            <p className="font-semibold text-slate-700 mb-1">雲端連線狀態：</p>
            {user ? (
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans pb-12">
      ...
      {toastMsg.show && (
        <div
          className={`fixed top-20 left-1/2 transform -translate-x-1/2 px-6 py-3 rounded-2xl shadow-xl z-50 text-white font-bold text-sm flex items-center gap-2 animate-bounce ${
          ...
          <div className="text-xs bg-slate-800 border border-slate-700 px-3 py-1.5 rounded-full text-slate-300 flex items-center">
            <span
              className={`inline-block w-2 h-2 rounded-full mr-2 ${
                user ? 'bg-emerald-400 animate-pulse' : 'bg-rose-500'
              }`}
            ></span>
            {user ? '雲端連線中' : '斷線'}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex space-x-2 overflow-x-auto py-2.5 no-scrollbar">
          {[
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 text-sm font-bold rounded-xl whitespace-nowrap transition ${
                activeTab === tab.id
